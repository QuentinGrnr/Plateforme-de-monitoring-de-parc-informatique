...
sqlite3 -header -csv "$DB_PATH" "SELECT date_heure_scan, cpuUsage FROM monitoring ORDER BY date_heure_scan DESC LIMIT $LIMIT;" > data.csv

# Utiliser gnuplot pour générer le graphique
gnuplot << EOF
set datafile separator ","
set terminal png size 800,600
set output "Web/static/img/cpu_usage.png"
set xdata time
set timefmt "%Y-%m-%d %H:%M:%S"
set format x "%H:%M"  # Réglez le format de l'axe x comme nécessaire
set xlabel "Date et heure" textcolor rgb "white"
set ylabel "CPU Usage (%)" textcolor rgb "white"
set title "Evolution du CPU Usage" textcolor rgb "white"
...
plot "data.csv" using 1:2 with lines ls 1 title "CPU Usage"
EOF
